...
const Bitgetbot = () => {
  ...
  useEffect(() => {
    const socket = io('https://reivun-gkdi.vercel.app');  // Connect to the backend WebSocket server

    socket.on('data', (data) => {
      setSymbolsData(data);  // Update symbols data from the backend
    });
  ...
  return (
    <div>
      <h1>Bitget Bot</h1>
      <div>
        {/* Render your symbols data */}
        {Object.entries(symbolsData).map(([symbol, data]) => (
          <div key={symbol}>
            <h2>{symbol}</h2>
            <p>Close: {data.close}</p>
            <p>Is Hammer: {data.isHammer ? 'Yes' : 'No'}</p>
          </div>
        ))}
      </div>
    </div>
